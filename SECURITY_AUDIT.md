...
### M5. Слабый дефолтный `ADMIN_PASSWORD`
**Файл:** Production `.env` на VPS

Дефолтный пароль был словарным — защищался только тем что путь `/admin*` не публикуется. Реалистичный риск — leak логов / process-list / сосед на VPS.
